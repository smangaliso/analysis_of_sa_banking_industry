...
def run(parent_url: str = None):
    ...
    for data_directory in data_directories:
        get_mortgages(data_directory=data_directory, output=output)

    return output


def get_mortgages(data_directory: str = None, output: dict = None):
    """
    ...
    for bank_code in BankCodes:
        ...
        # get_mortgages
        mortgage = data_reader.select(column_id="Item Number", search_value=155)["TOTAL ASSETS (Col 1 plus col 3)"].values[0]

        #update output

        try:
            output[month_id][bank_code.name] = mortgage
